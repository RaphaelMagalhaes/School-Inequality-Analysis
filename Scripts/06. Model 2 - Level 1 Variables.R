...
fe.read.m2 <- data.frame(Coef = coef.fw,
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.read.m2 <- data.frame(model = "Students Variables",
                             area = "Reading",
...
# assembling and saving coefficients results
fe.math.m2 <- data.frame(Coef = coef.fw,
                         S.E. = sqrt(var.coef+(1.1*var.imp)),
                         `t-value` = coef.fw/sqrt(var.coef+(1.1*var.imp)),
                         `p-value` = round(1-pt(abs(coef.fw/sqrt(var.coef+(1.1*var.imp))), 
                                                df = Inf), 
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.math.m2 <- data.frame(model = "Students Variables",
                             area = "Mathematics",
...
# assembling and saving coefficients results
fe.scie.m2 <- data.frame(Coef = coef.fw,
                         S.E. = sqrt(var.coef+(1.1*var.imp)),
                         `t-value` = coef.fw/sqrt(var.coef+(1.1*var.imp)),
                         `p-value` = round(1-pt(abs(coef.fw/sqrt(var.coef+(1.1*var.imp))), 
                                                df = Inf), 
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.scie.m2 <- data.frame(model = "Students Variables",
                             area = "Sciences",
...
fe <- rbind(fe.read.m2,
            fe.math.m2,
            fe.scie.m2)
export(fe, "Results/13. Fixed Effects in Model 2.csv")

# graphically
fe.graph <- left_join(left_join(fe.read.m2, 
                    fe.math.m2, 
                    by = "Variable"),
          fe.scie.m2,
          by = "Variable") %>% 
...
cbind(Variable = fe.graph$Variable, round(fe.graph[,2:13],2)) %>% 
  ...
  tab_header(title = md("**Fixed Effects - Model 2**")) %>% 
...
## ICC
icc.m2 <- data.frame(Area = c("Reading",
                                "Mathematics",
                                "Sciences"),
                       ICC = c(icc.read.m2,
                               icc.math.m2,
                               icc.scie.m2))
export(icc.m2, "Results/14. ICC in Model 2.csv")
...
icc.m2 %>% 
  ...
  tab_header(title = md("**ICC - Model 2**"))


## Random Effects
re.m2 <- rbind(re.read.m2,
               re.math.m2,
               re.scie.m2)
export(re.m2, "Results/15. Random Effects in Model 2.csv")
...
left_join(re.m2, sch.type) %>% 
  ...
  labs(title = "Random Effects - Model 2",
       y = "Schools",
       x = "Random Effects") +
