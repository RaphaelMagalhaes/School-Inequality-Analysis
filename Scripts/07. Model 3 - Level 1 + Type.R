...
fwread <- lapply(1:10,
                 function(pv)
                   lmer(pisa[,0+pv] ~ SEX + GRADE + ISCEDO + 
                          DISCLIMA + ESCS + SCHLTYPE + (1|CNTSCHID),
                        data = pisa,
                        weights = std_wgt))
...
fe.read.m3 <- data.frame(Coef = coef.fw,
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.read.m3 <- data.frame(model = "School Type",
                             area = "Reading",
...
# assembling and saving coefficients results
fe.math.m3 <- data.frame(Coef = coef.fw,
                         S.E. = sqrt(var.coef+(1.1*var.imp)),
                         `t-value` = coef.fw/sqrt(var.coef+(1.1*var.imp)),
                         `p-value` = round(1-pt(abs(coef.fw/sqrt(var.coef+(1.1*var.imp))), 
                                                df = Inf), 
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.math.m3 <- data.frame(model = "School Type",
                             area = "Mathematics",
...
# assembling and saving coefficients results
fe.scie.m3 <- data.frame(Coef = coef.fw,
                         S.E. = sqrt(var.coef+(1.1*var.imp)),
                         `t-value` = coef.fw/sqrt(var.coef+(1.1*var.imp)),
                         `p-value` = round(1-pt(abs(coef.fw/sqrt(var.coef+(1.1*var.imp))), 
                                                df = Inf), 
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.scie.m3 <- data.frame(model = "School Type",
                             area = "Sciences",
...
cbind(Variable = fe.graph$Variable, round(fe.graph[,2:13],2)) %>% 
  ...
  tab_header(title = md("**Fixed Effects - Model 3**")) %>% 
...
## ICC
icc.m3 <- data.frame(Area = c("Reading",
                                "Mathematics",
                                "Sciences"),
                       ICC = c(icc.read.m3,
                               icc.math.m3,
...
icc.m3 %>% 
  ...
  tab_header(title = md("**ICC - Model 3**"))
