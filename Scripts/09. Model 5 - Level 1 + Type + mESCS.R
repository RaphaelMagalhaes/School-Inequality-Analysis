...
fwread <- lapply(1:10,
                 function(pv)
                   lmer(pisa[,0+pv] ~ SEX + GRADE + ISCEDO + 
                          DISCLIMA + ESCS + SCHLTYPE + mESCS + (1|CNTSCHID),
                        data = pisa,
                        weights = std_wgt))
...
fe.read.m5 <- data.frame(Coef = coef.fw,
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.read.m5 <- data.frame(model = "Type + mESCS",
                             area = "Reading",
...
# assembling and saving coefficients results
fe.math.m5 <- data.frame(Coef = coef.fw,
                         S.E. = sqrt(var.coef+(1.1*var.imp)),
                         `t-value` = coef.fw/sqrt(var.coef+(1.1*var.imp)),
                         `p-value` = round(1-pt(abs(coef.fw/sqrt(var.coef+(1.1*var.imp))), 
                                                df = Inf), 
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.math.m5 <- data.frame(model = "Type + mESCS",
                             area = "Mathematics",
...
# assembling and saving coefficients results
fe.scie.m5 <- data.frame(Coef = coef.fw,
                         S.E. = sqrt(var.coef+(1.1*var.imp)),
                         `t-value` = coef.fw/sqrt(var.coef+(1.1*var.imp)),
                         `p-value` = round(1-pt(abs(coef.fw/sqrt(var.coef+(1.1*var.imp))), 
                                                df = Inf), 
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.scie.m5 <- data.frame(model = "Type + mESCS",
                             area = "Sciences",
...
fe <- rbind(fe.read.m5,
            fe.math.m5,
            fe.scie.m5)
export(fe, "Results/22. Fixed Effects in Model 5.csv")

# graphically
fe.graph <- left_join(left_join(fe.read.m5, 
                    fe.math.m5, 
                    by = "Variable"),
          fe.scie.m5,
          by = "Variable") %>% 
...
cbind(Variable = fe.graph$Variable, round(fe.graph[,2:13],2)) %>% 
  ...
  tab_header(title = md("**Fixed Effects - Model 5**")) %>% 
...
## ICC
icc.m5 <- data.frame(Area = c("Reading",
                                "Mathematics",
                                "Sciences"),
                       ICC = c(icc.read.m5,
                               icc.math.m5,
                               icc.scie.m5))
export(icc.m5, "Results/23. ICC in Model 5.csv")
...
icc.m5 %>% 
  ...
  tab_header(title = md("**ICC - Model 5**"))


## Random Effects
re.m5 <- rbind(re.read.m5,
               re.math.m5,
               re.scie.m5)
export(re.m5, "Results/24. Random Effects in Model 5.csv")
...
left_join(re.m5, sch.type) %>% 
  ...
  labs(title = "Random Effects - Model 5",
       y = "Schools",
       x = "Random Effects") +
