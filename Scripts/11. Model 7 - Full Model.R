...
fwread <- lapply(1:10,
                 function(pv)
                   lmer(pisa[,0+pv] ~ SEX + GRADE + ISCEDO + 
                          DISCLIMA + ESCS + SCHLTYPE + PROATCE + mESCS + mDISCLIMA + (1|CNTSCHID),
                        data = pisa,
                        weights = std_wgt))
...
fe.read.m7 <- data.frame(Coef = coef.fw,
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.read.m7 <- data.frame(model = "Type + PROATCE + mESCS + mDISCLIMA",
                             area = "Reading",
...
# assembling and saving coefficients results
fe.math.m7 <- data.frame(Coef = coef.fw,
                         S.E. = sqrt(var.coef+(1.1*var.imp)),
                         `t-value` = coef.fw/sqrt(var.coef+(1.1*var.imp)),
                         `p-value` = round(1-pt(abs(coef.fw/sqrt(var.coef+(1.1*var.imp))), 
                                                df = Inf), 
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.math.m7 <- data.frame(model = "Type + PROATCE + mESCS + mDISCLIMA",
                             area = "Mathematics",
...
# assembling and saving coefficients results
fe.scie.m7 <- data.frame(Coef = coef.fw,
                         S.E. = sqrt(var.coef+(1.1*var.imp)),
                         `t-value` = coef.fw/sqrt(var.coef+(1.1*var.imp)),
                         `p-value` = round(1-pt(abs(coef.fw/sqrt(var.coef+(1.1*var.imp))), 
                                                df = Inf), 
...
for(pv in 1:10){
  ...
  if(pv == 10){
    re.scie.m7 <- data.frame(model = "Type + PROATCE + mESCS + mDISCLIMA",
                             area = "Sciences",
...
cbind(Variable = fe.graph$Variable, round(fe.graph[,2:13],2)) %>% 
  ...
  tab_header(title = md("**Fixed Effects - Full Model**")) %>% 
...
## ICC
icc.m7 <- data.frame(Area = c("Reading",
                              "Mathematics",
                              "Sciences"),
                     ICC = c(icc.read.m7,
                             icc.math.m7,
...
icc.m7 %>% 
  ...
  tab_header(title = md("**ICC - Full Model**"))
...
left_join(re.m7, sch.type) %>% 
  ...
  labs(title = "Random Effects - Full Model",
       y = "Schools",
       x = "Random Effects") +
